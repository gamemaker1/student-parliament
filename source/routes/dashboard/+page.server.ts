...
export const load: PageServerLoad = async (event) => {
	...
	const [announcements, issues, committees] = await Promise.all([
		...
		db.select().from(table.issue).orderBy(desc(table.issue.modified)).limit(20),
		db
			.select(committeeQuery)
			.from(table.committee)
			.leftJoin(table.user, eq(table.user.committeeId, table.committee.id))
			.groupBy(table.committee.id)
			.orderBy(desc(table.committee.name)),
	])
